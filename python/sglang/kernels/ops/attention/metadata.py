...
def _fused_metadata_kernel_general(
    ...
    # Unified pool: the source is already translated. No >>SHIFT, no v2p,
    # no mapping gather.
    SRC_IS_TRANSLATED: tl.constexpr = 0,
):
    ...
    # when the source is already translated and thus page-granular)
    if SRC_IS_TRANSLATED:
        col_idx = col_offsets
    elif page_size == 1:
        col_idx = col_offsets
    else:
...
def normal_decode_set_metadata(
    ...
    Unified pool (``src_is_translated=True``): ``req_to_token`` /
    ``req_pool_indices`` are already kernel-facing; ``swa_src_table`` has the
    same shape and strides; steps 3-5 become verbatim copies of the rows'
    live prefixes.
    ...
    if page_size == 1 and not use_swa:
        ...
        # SWA parameters
        if use_swa and src_is_translated:
        ...
        _fused_metadata_kernel_general[grid](
            ...
            shift,
            BLOCK_COLS=BLOCK_COLS,
            SRC_IS_TRANSLATED=1 if src_is_translated else 0,
            num_warps=4,
            num_stages=3,
        )
