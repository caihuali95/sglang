...
def normal_decode_set_metadata(
    ...
    src_is_kernel_page_table: bool = False,
    ...
    Unified pool (``src_is_kernel_page_table=True``): ``req_to_token`` /
    ...
    # SHIFT, no gather transform), so src_is_kernel_page_table needs no
    ...
    if page_size == 1 and not use_swa:
        ...
        # SWA parameters
        if use_swa and src_is_kernel_page_table:
            ...
            assert swa_src_table.stride() == req_to_token.stride(), (
        ...
        _fused_metadata_kernel_general[grid](
            ...
            shift,
            BLOCK_COLS=BLOCK_COLS,
            SRC_IS_KERNEL_PAGE_TABLE=1 if src_is_kernel_page_table else 0,
            num_warps=4,
            num_stages=3,
        )
