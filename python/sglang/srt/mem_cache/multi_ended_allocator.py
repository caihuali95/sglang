...
def _float_open_short_side(flt, demand) -> None:
    ...
    for band_alloc, pages in demand.items():
        if band_alloc is flt or pages <= 0:
            continue
        holes = (
            len(band_alloc._free_phys_pages) if band_alloc.lazy_compaction else 0
        )
    ...
    slack = max(
        b.entry_bytes_per_page for b, pages in demand.items() if pages > 0
    )
...
class MultiEndedAllocator(BaseTokenToKVPoolAllocator):
    ...
    def _ask_float_for_room(self, need_tokens: int) -> None:
        ...
        _float_open_short_side(
            blocker, {self: -(-need_tokens // self.page_size)}
        )
...
class FloatMultiEndedAllocator(MultiEndedAllocator):
    ...
    def _available_tokens(self, extra_gap_bytes: int = 0) -> int:
        ...
        pages_by_index_space = (
            self.num_pages - self.min_page_index - self._live_pages()
        )
    ...
    def make_room(self, *, side: str, min_bytes: int) -> int:
        ...
        if side == "high":
            ...
            usable_holes = sorted(
                (h for h in holes if h > max(srcs)), reverse=True
            )
    ...
    def _relocate_to_positions(
        self, live_sorted: List[int], final: List[int]
    ) -> int:
        ...
        if pairs:
            ...
            if src_set.isdisjoint(dst_set):
                ...
                ordered = pairs if final[0] <= live_sorted[0] else list(
                    reversed(pairs)
                )
                for s, d in ordered:
    ...
    def _byte_accounting_violations(self) -> List[str]:
        ...
        total = self.unified_buffer.total_bytes
        lo_b, hi_b = self._byte_low_frontier(), self._byte_high_frontier()
        if not self._is_frontier_transparent() and not (
            0 <= lo_b <= hi_b <= total
        ):
            out.append(
        ...
        if self._span_pages() > 0:
            bound = int(
                (
                    self.physical_to_virtual[self.low_wm_page : self.high_wm_page]
                    != -1
                )
                .sum()
                .item()
    ...
    def compact_holes(self, *, retreat_side: str) -> int:
        ...
        if retreat_side == "high":
            ...
            final = list(
                range(self.high_wm_page - len(live_sorted), self.high_wm_page)
            )
...
class UnifiedSWATokenToKVPoolAllocator(SWATokenToKVPoolAllocator):
    ...
    def _release_page_reps(self, pieces: Sequence[torch.Tensor]) -> None:
        ...
        if live_pages.numel() > 0:
            self.swa_attn_allocator.free(
                live_pages * self.page_size, _pages=live_pages
            )
...
class UnifiedMambaSWATokenToKVPoolAllocator(UnifiedSWATokenToKVPoolAllocator):
    ...
    def restore_state(self, state):
        assert len(state) == 3
        rollback = UnifiedSWATokenToKVPoolAllocator.restore_state(
            self, state[:2]
        )
