...
def alloc_req_slots(
    ...
    mid-loop — the eval_131 Bug #2 family of crashes.
    """
    num_reqs = len(reqs)
    if isinstance(req_to_token_pool, HybridReqToTokenPool):
        ...
        # The eviction-trigger factor: how much "headroom" to ask the
        # tree cache to free. For radix this is 3× to leave room for
        # potential COW (mamba_radix_cache._match_post_processor may
        # allocate 1-2 additional slots per req for copy-on-write
        # branching). For chunk this is 1× — exact required.
        evict_factor = (
        ...
        # turning into a livelock under steady-state pressure
        # (observed at eval_133 Falcon mfs=0.45 radix with all reqs
        # pinning mamba slots: planner kept refusing, scheduler kept
        # restoring, decoding froze).
        ...
        if mamba_available_size < evict_target:
            if tree_cache is not None and tree_cache.supports_mamba():
                ...
                # freed enough for the strict minimum.
                mamba_available_size = getattr(
                    mamba_pool,
