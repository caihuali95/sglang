...
consume a `KVIndexBatchView` and stay id-space-blind:
    ...
    non-unified: table = req_to_token, rows = req_pool_indices — the EXACT
                 ...
                 passthrough (the F2 no-op precedent).
...
Capture contract (the F2 write-loc precedent): capture-stable buffers are
...
class KVIndexSource:
    ...
    def rebind_write_loc(self, forward_batch) -> None:
        """The WRITE half of the id-space contract (the F2 rebind), folded
    ...
    def translate_swa(
        ...
        """Virtual token ids -> kernel-facing swa-side ids."""
        assert self._translate_swa is not None, (
            "translate_swa on a source without a unified SWA side"
        )
        return self._translate_swa(kv_indices, out=out)
