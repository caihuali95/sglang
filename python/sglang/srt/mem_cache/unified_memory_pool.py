...
from typing import ClassVar, Dict, List, NamedTuple, Optional, Sequence, Tuple
...
def _reserved_floor_bytes(
    sub_pool_specs: "List[SubPoolSpec]", page_size: int
) -> int:
...
class UnifiedSWAKVPool(SWAKVPool):
    ...
    def set_kv_buffer(
        ...
        if is_swa:
            ...
            return
        # Full layer: post-F2 the generic `loc` IS the full-side kernel-facing
        ...
        # (eval_455: fa3 x unified-SWA hit the old must-be-precomputed assert
        # at gpt-oss cuda-graph capture).
        if full_loc is None:
            full_loc = loc
...
def init_unified_mamba_swa_pools(
    *,
        ...
        floor_terms=[
            ...
            (
                ...
                _reserved_floor_bytes(
                    [full_spec, swa_spec, mamba_spec], page_size
                ),
            ),
        ],
        factory="init_unified_mamba_swa_pools",
    )
    # Uniform-row full AND swa sub-pools get DENSE views; the mamba/conv state
    # sub-pool ALWAYS stays strided (heterogeneous rows; its kernels are
    # stride-aware in-tree code).
    dense_mha = unified_dense_mha_enabled(full_spec, swa_spec)
    shared_pool = UnifiedKVPool(
        total_bytes=total_bytes,
    ...
    logger.info(
        ...
        "head_num=%d/%d, head_dim=%d/%d, page_size=%d (%s)",
        len(full_attention_layer_ids),
        len(swa_attention_layer_ids),
        len(mamba_layer_ids),
        head_num,
