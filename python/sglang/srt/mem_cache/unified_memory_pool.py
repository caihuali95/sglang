...
class MHASubPoolSpec(SubPoolSpec):
    ...
    `dense_blocks_per_page()` (host) and `draft_kernel_page_multiplier()`
    ...
    def dense_blocks_per_page(self) -> int:
        ...
        integral by the `entry_bytes` padding. Uniform rows are guaranteed by
        `__post_init__`, so the block array always exists."""
...
class UnifiedKVPool:
    ...
    def build_dense_draft_views(
        ...
        _assert_dense_id_bound(
            ...
            n_dense=num_pages * spec.draft_kernel_page_multiplier() * page_size,
        )
        return build_dense_mha_views(
            self._raw,
...
class UnifiedDraftKVPool(MHATokenToKVPool):
    ...
    id-space choke point binds it — see KVIndexSource). Relocation needs no
    ...
    def __init__(
        ...
        *,
        ...
        # The id-space choke point (KVIndexSource) probes these: the draft's
