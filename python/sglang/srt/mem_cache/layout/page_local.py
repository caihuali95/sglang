"""Page-granularity envelope (page-local layer-major) cache views.
...
def build_page_local_mha_views(
    raw: torch.Tensor,
    *,
    layer_num: int,
    head_num: int,
    ...
    """Per-layer K/V views over ``raw`` in the page-local layer-major layout.
...
def build_mamba_envelope_views(
    raw: torch.Tensor,
    *,
    layer_num: int,
    conv_state_shapes: Sequence[Sequence[int]],
    ...
    ) + c_strides(temporal_state_shape)
    temporal_view = torch.as_strided(
        as_temporal_dtype,
        size=(n, max_slots) + tuple(temporal_state_shape),
        stride=stride,
        storage_offset=offset_elems,
    )
