...
def build_dense_mha_views(
    ...
    *,
    ...
    starts ``region_offset_bytes`` into each page. The dense id space scales
    with the stride — ``dense(t) = (t // ps) * (ps * page_stride_blocks)
    ...
    assert region_offset_bytes % itemsize == 0, (
        f"build_dense_mha_views: region_offset_bytes={region_offset_bytes} must "
...
def build_dense_mla_views(
    ...
    *,
    ...
    kv_cache_dim)`` tensor, addressed by the layer-independent dense id
        ...
        dense(t) = (t // ps) * (ps * page_stride_rows) + t % ps
    ...
    n_dense = num_pages * page_stride_rows * page_size
    assert anchor_bytes % itemsize == 0
    last_view_end = (
