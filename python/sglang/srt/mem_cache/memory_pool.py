...
class HybridLinearKVPool(KVCache):
    ...
    def __init__(
        ...
        # receive KERNEL-FACING ids on every pool: writes carry the
        # ForwardBatch's rebound `out_cache_loc` (apply_unified_kv_loc_rebind,
        # dense-first), and read indices are translated at their production
    ...
    def set_mla_kv_buffer(
        ...
        # Model-side entry point: `loc` is `forward_batch.out_cache_loc`,
        # which is KERNEL-FACING on every pool — non-unified pools allocate
        # physical ids, and the unified pool rebinds the ForwardBatch at
        # construction (apply_unified_kv_loc_rebind) using the same
        # dense-first preference the attention backends use
        # (translate_kv_loc_dense falls back to translate_kv_loc). The pool
        # never translates here; a caller handing virtual ids would already
        # have tripped the backend's physical-contract assert.
        with self._transfer_id_context(layer):
    ...
    def get_mla_kv_buffer(
        ...
        # Model-side entry point — same kernel-facing contract as
        # set_mla_kv_buffer: `loc` is a read-index tensor already translated
        # at its production site (fetch_mha_one_shot_kv_indices /
