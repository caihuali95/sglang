            ...
            if self.enable_overlap:
                ...
                with self.forward_stream_ctx:
                    ...
                    with self._forward_isolation(batch, overlap=True):
                        ...
                        # enqueued at this point, and nothing else (frees of the
                        # previous batch, new allocations) has run yet —
                        # allocators that relocate pages under in-flight forwards
                        # (the unified pool's lazy compaction) rely on being told
                        # HERE, before any flush can consider their pages. Base
                        # no-op for every other allocator. Fused draft KV is
                        # covered by construction: the write-set is slot/page-
                        # granular and the draft writes its byte region at the
                        # SAME out_cache_loc ids the target writes.
