...
class Envs:
    ...
    # Fuse the draft model's KV into the unified full-KV slot entry
    # ([target KV | draft KV], Stage 4.1 / design doc §33.4 Design B),
    # replacing the virtual-index-sized private draft pool + its reserve.
    # Bring-up opt-in; becomes default-on (renamed to the
    # SGLANG_DISABLE_FUSED_DRAFT_KV escape hatch) once GPU-eval-validated.
    SGLANG_ENABLE_FUSED_DRAFT_KV = EnvBool(False)
