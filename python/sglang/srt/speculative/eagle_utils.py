...
def organize_draft_results(
    ...
    # inconsistent tree — its device-side parent walk then runs out of bounds
    # or spins forever (a GPU hang, not an error). Detect loudly (hard assert
    # under SGLANG_ENABLE_ASYNC_ASSERT, throttled warning otherwise), then
    # rank NaN below every valid score (valid scores are cumulative
    # probabilities, >= 0).
