...
def apply_unified_kv_loc_rebind(fb, model_runner) -> None:
    ...
    """
    translate = getattr(
        model_runner.token_to_kv_pool_allocator, "translate_kv_loc", None
    )
...
class ForwardBatch(ForwardBatchDeepSeekMHAMixin):
    ...
    # True once out_cache_loc has been rebound to PHYSICAL ids
    # (apply_unified_kv_loc_rebind). Backends with a wired translate handle
    # assert this — a hand-built ForwardBatch that skipped the rebind fails
    # loudly instead of writing virtual ids as if physical.
