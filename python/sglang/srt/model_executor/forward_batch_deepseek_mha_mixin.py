...
class ForwardBatchDeepSeekMHAMixin:
    ...
    def prepare_chunked_kv_indices(self, device: torch.device):
        ...
        for idx in range(self.num_prefix_chunks):
            ...
            # PHYSICAL, mirroring the write rail's ForwardBatch rebind. The
            # translate returns int64 (the v2p dtype) — exactly what the door
            # used to hand downstream. No-op on non-unified pools (callable is
            # None; the int32 indices are already physical there).
    ...
    def fetch_mha_one_shot_kv_indices(self):
        ...
        # Unified pool: req_to_token stores VIRTUAL ids — translate at
        # production (once; the cache below holds the PHYSICAL result and this
        # method has a single consumer) so the pool door (get_mla_kv_buffer)
        # receives physical, mirroring the write rail's ForwardBatch rebind.
        # The translate returns int64 (the v2p dtype) — exactly what the door
        # used to hand downstream. No-op on non-unified pools (callable None).
        if self._unified_kv_loc_translate is not None:
