...
class ForwardBatchDeepSeekMHAMixin:
    ...
    def prepare_chunked_kv_indices(self, device: torch.device):
        ...
        for idx in range(self.num_prefix_chunks):
            ...
            # req_to_token holds VIRTUAL ids. Translate here as the pool
            # door receives kernel-facing ids.
            translator = get_attn_backend().kv_index_translator
            if translator is not None:
                chunk_kv_indices = translator.translate_full_attn_ids(chunk_kv_indices)
    ...
    def fetch_mha_one_shot_kv_indices(self):
        ...
        translator = get_attn_backend().kv_index_translator
        if translator is not None:
            kv_indices = translator.translate_full_attn_ids(kv_indices)
