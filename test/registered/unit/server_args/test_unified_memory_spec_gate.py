...
class TestUnifiedMemorySpecGate(unittest.TestCase):
    ...
    def test_host_pool_retraction_refused(self):
        """The host-pool retraction backup builds host pools off the draft's
        device pool even without HiCache; a fused draft view has no transfer
        surface, so the backup is refused with the other host tiers."""
        self.assertFalse(_accepts("EAGLE", retraction_backup="host_pool"))
        self.assertFalse(_accepts(None, retraction_backup="host_pool"))
