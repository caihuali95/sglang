...
class TestUnifiedTranslateBanned(CustomTestCase):
    ...
    def test_no_write_loc_translator_calls(self):
        """The write half has no backend-callable translator API at all:
        phase 1 (the full-side rebind) runs at ForwardBatch construction, and
        phase 2 (the sliding-window write loc) is a field on the per-batch
        index table, derived from dense values. The retired stateful surfaces
        (`sliding_window_write_loc`, `adopt_replaced_write_loc`) must not
        regrow as backend calls — per-backend write-loc handling is how both
        silent id-space failure modes were built the first time."""
        banned = re.compile(r"\.(sliding_window_write_loc|adopt_replaced_write_loc)\(")
        hits = [
            f"{rel}: {m.group(0)}"
            for rel, src in _iter_sources()
            for m in banned.finditer(src)
