...
`translate_kv_loc_dense` / KVIndexTranslator machinery serve the draft with just
...
from sglang.srt.mem_cache.layout.page_major import build_dense_mha_views
...
class TestFusedSpecMath(unittest.TestCase):
    def test_unfused_spec_is_byte_identical_to_before(self):
        ...
        self.assertEqual(s.dense_blocks_per_page(), 4)
    ...
    def test_fused_entry_is_lcm_padded_and_both_strides_integral(self):
        ...
        self.assertEqual(f.dense_blocks_per_page(), f.entry_bytes() // w_h)
...
class TestGeneralizedBuilderDefaults(unittest.TestCase):
    def test_default_params_reproduce_the_unfused_views(self):
        ...
        k0, v0 = build_dense_mha_views(raw, **kwargs)
        k1, v1 = build_dense_mha_views(
...
class TestFusedRegionConfinement(unittest.TestCase):
    ...
    def _build(self):
        ...
        hk, hv = build_dense_mha_views(
            ...
            page_stride_blocks=spec.dense_blocks_per_page(),
        )
        r = spec.draft_region
        dk, dv = build_dense_mha_views(
            raw,
    ...
    def test_host_and_draft_writes_confine_to_their_regions(self):
        ...
        for page in range(self.PAGES):
            for off in range(self.PS):
                token_dense_h = page * self.PS * spec.dense_blocks_per_page() + off
                token_dense_d = (
...
class TestFusedMLAHost(unittest.TestCase):
    ...
    def test_unfused_spec_is_byte_identical_to_before(self):
        ...
        self.assertEqual(s.dense_blocks_per_page(), 2)
        self.assertEqual(s.view_tail_pad_bytes(4), 4 * s.entry_bytes())
    ...
    def test_host_and_draft_writes_stay_byte_disjoint_within_a_page(self):
        ...
        for layer_view in host_views:
            raw.zero_()
            layer_view[page * self.PS * full.dense_blocks_per_page()] = 1.0
            nz = raw.nonzero()
            self.assertGreater(nz.numel(), 0)
    ...
    def test_translator_takes_the_fused_disposition_on_the_mla_host(self):
        ...
        alloc = UnifiedMambaTokenToKVPoolAllocator(
            ...
            full_kernel_page_multiplier=pool.mla_spec("full").dense_blocks_per_page(),
        )
        dp = UnifiedDraftKVPool(
            unified_buffer=pool,
            host_sub_pool_name="full",
