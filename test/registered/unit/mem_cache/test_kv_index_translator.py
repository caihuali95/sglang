...
Triton kernel is a later CUDA CI pin):
  ...
  - the two-phase write contract: the rebind touches only the full side, and
    the sliding-window write loc derives POINTWISE from the dense values
...
class TestPoolOwnership(unittest.TestCase):
    ...
    kernel-facing space (dense ids up to num_pages * multiplier) and then used
...
class _FakeForwardBatch:
    ...
    `index_table_for_batch` and `rebind_write_loc` read."""
...
class TestWriteLoc(unittest.TestCase):
    ...
    def _field(self, src, rows, seq_lens, dense_loc):
        return src.build_index_table(
            req_pool_indices=rows,
            seq_lens=seq_lens,
            max_pages=4,
            out_cache_loc=dense_loc,
