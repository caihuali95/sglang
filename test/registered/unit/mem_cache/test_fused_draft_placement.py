...
prices. Pinned:
  - a replicated head under multi-layer EAGLE gets one slot RANGE per runner
...
class TestPlaceFusedDraft(CustomTestCase):
    def test_replicated_head_gets_one_slot_range_per_runner(self):
        ...
        self.assertEqual(placement.full.layer_num, 6)
        self.assertEqual(placement.hosts(), ("full",))
        for r in range(3):
            self.assertEqual(placement.slots_for(r, "full"), range(2 * r, 2 * r + 2))
            self.assertEqual(placement.slots_for(r, "swa"), range(0))
    ...
    def test_region_carries_the_profile_geometry(self):
        ...
        self.assertEqual(
            placement.full,
            DenseDraftRegion(
                layer_num=1, head_num=4, head_dim=64, v_head_dim=64, store_dtype=_DTYPE
            ),
        )
...
class TestFusedDraftPlacement(CustomTestCase):
    def test_runner_ranges_must_tile_the_region(self):
        region = DenseDraftRegion(
            layer_num=2, head_num=1, head_dim=8, store_dtype=_DTYPE
        )
