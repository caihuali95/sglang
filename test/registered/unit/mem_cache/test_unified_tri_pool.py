...
class TestUnifiedTriPool(unittest.TestCase):
    ...
    def test_mamba_slot_allocator_surface(self):
        ...
        slot_alloc.free(v)
        self.assertEqual(
            slot_alloc.available_size(), pool.max_slots("mamba") - 1
        )
    ...
    def test_mamba_slot_full_token_cost_formula(self):
        ...
        self.assertEqual(
            allocator.mamba_slot_full_token_cost(), -(-m // e_tok)
        )
    ...
    def test_urgent_flush_preserves_float_holes(self):
        ...
        from sglang.srt.mem_cache.multi_ended_allocator import _relieve_for_alloc; _relieve_for_alloc(allocator, 1)
...
class TestGeneralizedRebalance(unittest.TestCase):
    ...
    def test_direction_is_derived_from_growth_on_both_ends(self):
        ...
        inst = TestFloatMultiEndedAllocator(
            [
                m
                for m in dir(TestFloatMultiEndedAllocator)
        ...
        fit = int(
            (down_end._byte_low_frontier() - fla._byte_high_frontier()) // e_dn
        )
        if fit > 0:
...
class TestComputedShortSide(unittest.TestCase):
    ...
    def _sides(self, alloc):
        sa = alloc.swa_attn_allocator
        low = max(
            0, sa._byte_low_frontier() - sa._chain_high_frontier_below_bytes()
        )
        high = max(
            0, sa._chain_low_frontier_above_bytes() - sa._byte_high_frontier()
        )
        return low, high
    ...
    def test_full_side_short_target_matches_the_closed_form(self):
        ...
        calls = []
        with mock.patch.object(sa, "make_room", side_effect=lambda **kw: calls.append(kw)):
            alloc._ask_float_for_room(chosen * self.PS)
...
class TestFloatPolicyTotalTarget(unittest.TestCase):
    ...
    def test_partial_gap_state_alloc_still_succeeds(self):
        ...
        e_m = ma.entry_bytes_per_page
        gap_slots = int(
            (sa._byte_low_frontier() - ma._byte_high_frontier()) // e_m
        )
...
class TestTriDeferredAbsorption(unittest.TestCase):
    ...
    def test_shortfall_ladder_absorbs_before_the_deficit_math(self):
        ...
        from sglang.srt.mem_cache.multi_ended_allocator import _relieve_for_alloc; _relieve_for_alloc(alloc, 1)  # the ladder
...
class TestTriPoolHardening(unittest.TestCase):
    ...
    def test_alloc_rebalances_a_blocking_float(self):
        ...
        moved = sum(
            int(s.numel()) for s, _, _ in sa._inverse_history[moves_before:]
        )
    ...
    def test_check_decode_capacity_retract_convergence(self):
        ...
        need = 16
        while not allocator.check_decode_capacity(
            num_tokens=need, tree_cache=None
        ):
            self.assertGreater(
                len(reqs), 1, "retract must converge before bs=1"
            )
