...
differ in how they consume it:
  ...
  - fa3's captured decode: `normal_decode_set_metadata` copies the canonical
    rows' live prefixes (src_is_translated=True).
...
def _fill_block_table(
    ...
    (exactly what KVIndexTranslator.fill_read_table does for trtllm_mla/flashmla)."""
...
class TestFa3MetadataDenseBlockTable(unittest.TestCase):
    ...
    (src_is_translated=True): the fused kernel copies the canonical
