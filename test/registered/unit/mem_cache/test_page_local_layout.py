...
def _make_mha_views(layer_num, head_num, head_dim, v_head_dim, page_size, num_pages):
    ...
    k, v = build_page_local_mha_views(
        raw,
        layer_num=layer_num,
        head_num=head_num,
        head_dim=head_dim,
...
class TestPageLocalMHAViews(unittest.TestCase):
...
class TestPageLocalMove(unittest.TestCase):
...
class TestMambaEnvelopeViews(unittest.TestCase):
    def test_conv_temporal_shapes_no_alias(self):
        ...
        conv_views, temporal = build_mamba_envelope_views(
            raw,
            layer_num=layers,
            conv_state_shapes=conv_shapes,
            conv_dtype=conv_dt,
