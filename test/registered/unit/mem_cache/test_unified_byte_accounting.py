...
class TestStrictEnvContract(unittest.TestCase):
    def test_strict_check_defaults_off(self):
        """Escalation-to-crash is a validation posture; flipping the default
        changes production behavior and must be a deliberate act."""
        with envs.SGLANG_ENABLE_STRICT_UNIFIED_BYTE_CHECK.override(None):
            envs.SGLANG_ENABLE_STRICT_UNIFIED_BYTE_CHECK.clear()
            self.assertFalse(envs.SGLANG_ENABLE_STRICT_UNIFIED_BYTE_CHECK.get())
