"""Unit tests for the Stage 3.6 page-local layer-major byte layout.
...
1. The new 4-D ``_build_mha_views`` output exposes correct byte addresses
   ...
   degenerate ``page_size=1`` case (byte-identical to the Stage 1/2/3.5
...
class TestBuildMHAViews(unittest.TestCase):
    ...
    def test_strides_at_page_size_1_match_envelope(self):
        ...
        for L in range(spec.layer_num):
            ...
            self.assertEqual(
                k_views[L].stride(1), spec.k_row_bytes() // itemsize
            )
    ...
    def test_strides_at_page_size_gt_1(self):
        ...
        for L in range(spec.layer_num):
            ...
            self.assertEqual(
                k_views[L].stride(0), spec.page_bytes(8) // itemsize
            )
...
class TestMoveKVCacheNative4D(unittest.TestCase):
    ...
    def _build_buffer(self, page_size, layer_num=2, head_num=2, head_dim=4,
                      n_full_slots=64):
        full = _make_mha_spec(
            "full", "up", layer_num=layer_num, head_num=head_num, head_dim=head_dim
        )
...
class TestByteIdentityAtPageSize1(unittest.TestCase):
    ...
    physical bytes as Stage 1/2/3.5's old 3-D view would have. The view
    ...
    def test_byte_addresses_match_envelope(self):
        ...
        for L in range(spec.layer_num):
            for s in range(0, max_slots, max(1, max_slots // 4)):
                ...
                expected_k_byte_offset = (
                    s * spec.entry_bytes()
                    + L * (spec.k_row_bytes() + spec.v_row_bytes())
                )
