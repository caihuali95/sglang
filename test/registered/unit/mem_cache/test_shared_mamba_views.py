...
This isolates the shared-pool Mamba STATE layout from the full model. It is the
definitive localizer for the open shared-pool Mamba correctness defect
(design-doc §21.10): Falcon-H1 greedy decode is garbled under the shared pool,
isolated to the Mamba conv/temporal state path; the prime structural suspect is
the view construction (a stride/offset/alignment bug analogous to the fixed
