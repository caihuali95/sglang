...
class TestChainFrontierWalk(unittest.TestCase):
    ...
    def test_transparent_middle_is_skipped(self):
        ...
        self.assertEqual(
            fa._current_gap_bytes(), mid_lo - fa._byte_high_frontier()
        )
    ...
    def test_drainable_credit_reads_walked_neighbor(self):
        ...
        self.assertEqual(
            fa._peer_drainable_hole_bytes(), 3 * ma.entry_bytes_per_page
        )
...
class TestSWACompositeDenseSurface(unittest.TestCase):
    """The SWA composite's dense (kernel-facing) id surface (D3).

    Presence of `translate_kv_loc_dense` / `full_v2p_page_table` is what flips
    the dense-first probes (the F2 write rebind and the attention backends'
    ...
    def test_surface_collapses_at_multiplier_one(self):
        ...
        self.assertIs(
            a.full_v2p_page_table, a.full_attn_allocator.virtual_to_physical
        )
        self.assertIs(
            a.swa_v2p_page_table, a.swa_attn_allocator.virtual_to_physical
        )
        v = a.alloc(2 * self.PS)
        self.assertIsNotNone(v)
        self.assertTrue(
            torch.equal(a.translate_kv_loc_dense(v), a.translate_kv_loc(v))
        )
