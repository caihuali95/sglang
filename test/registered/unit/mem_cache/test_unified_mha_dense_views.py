...
class TestFactoryDenseViews(unittest.TestCase):
    ...
    def test_rebind_emits_dense_rails_from_virtual(self):
        """End-to-end over the real factory: rebind_write_loc must rebind
        out_cache_loc to FULL-DENSE ids and resolve the swa rail as SWA-DENSE
        ids — the swa formula over the VIRTUAL ids proves the order-critical
        rule (a rail computed after the rebind would gather v2p_swa at
        full-dense ids: garbage)."""
        ...
        self.assertTrue(
            torch.equal(source.resolve_swa_write_loc(fb.out_cache_loc), expected_swa)
        )
