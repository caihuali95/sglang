"""Triton-kernel parity test for the Stage 3.6 page-aware kernel changes.
...
class TestTritonKernelLayoutParity(unittest.TestCase):
    ...
    def _setup_decode_inputs(self, bs=2, head_num=2, head_dim=8, num_slots=64,
                              dtype=torch.float16):
        torch.manual_seed(0xC0FFEE)
        # Logical KV: shape [num_slots, head_num, head_dim]
        logical_kv_k = torch.randn(num_slots, head_num, head_dim, dtype=dtype,
                                   device="cuda")
        logical_kv_v = torch.randn(num_slots, head_num, head_dim, dtype=dtype,
                                   device="cuda")
    ...
    def _run_decode(self, q, k_buf, v_buf, kv_indptr, kv_indices, page_size):
        ...
        decode_attention_fwd(
            ...
            sm_scale=1.0 / (head_dim ** 0.5), k_scale=1.0, v_scale=1.0,
            logit_cap=0.0,
            page_size=page_size,
        )
        return o
    ...
    def test_extend_3d_vs_4d_ps1_byte_identical(self):
        ...
        k_buffer = torch.randn(num_slots, head_num, head_dim, dtype=dtype, device="cuda")
        v_buffer = torch.randn(num_slots, head_num, head_dim, dtype=dtype, device="cuda")
        q_extend = torch.randn(bs * extend_len, head_num, head_dim, dtype=dtype, device="cuda")
        k_extend = torch.randn(bs * extend_len, head_num, head_dim, dtype=dtype, device="cuda")
        v_extend = torch.randn(bs * extend_len, head_num, head_dim, dtype=dtype, device="cuda")
        ...
        kv_indices = torch.arange(prefix_len, dtype=torch.int64, device="cuda").repeat(bs)

        def run(k_buf, v_buf, page_size):
            o_out = torch.empty_like(q_extend)
            extend_attention_fwd(
                q_extend, k_extend, v_extend, o_out, k_buf, v_buf,
