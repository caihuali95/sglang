...
class TestKVIndexTranslatorDraftDisposition(unittest.TestCase):
    ...
    def test_sliding_window_read_ids_per_disposition(self):
        """`sliding_window_read_ids()` names the window gather source: the
        target's separate swa array when one exists, the SAME array under the
        fused draft's single-space swa, and the SAME array when untranslated
        ...
        self.assertIs(target.sliding_window_read_ids(), target.sliding_window_ids)
        ...
        self.assertIs(fused.sliding_window_read_ids(), fused.ids)

        raw = KVIndexTranslator(
            req_to_token=rt,
            token_to_kv_pool_allocator=allocator,
            token_to_kv_pool=_FakeKVCache(64),  # private pool: passthrough
            page_size=_PS,
            device=_DEV,
        ).build_index_table(req_pool_indices=rpi, seq_lens=seq, max_pages=2)
        self.assertFalse(raw.is_translated)
        self.assertIs(raw.sliding_window_read_ids(), raw.ids)
