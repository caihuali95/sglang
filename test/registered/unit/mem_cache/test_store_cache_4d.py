...
The kernel writes K/V into the 4-D page-local envelope view. These tests prove
...
class TestStoreCache4D(unittest.TestCase):
    ...
    def _check_parity(
        ...
        k_kernel, v_kernel, cache_k, cache_v, loc = self._make_view_and_cache(
            num_pages, page_size, head_num, head_dim, v_head_dim, N,
            dtype=dtype, loc_dtype=loc_dtype,
        )
    ...
    def test_store_cache_4d_ps1_byte_identical(self):
        ...
        self._check_parity(
            ...
            head_num=4, head_dim=128, v_head_dim=128,
            N=16,
        )
    ...
    def test_store_cache_4d_int32_loc(self):
        ...
        self._check_parity(
            num_pages=32, page_size=1,
            head_num=4, head_dim=64, v_head_dim=64,
            N=10,
    ...
    def test_store_cache_4d_empty_loc(self):
        ...
        k_view = torch.zeros(
            (8, 4, 4, 64), dtype=torch.bfloat16, device="cuda"
        )
        v_view = torch.zeros(
            (8, 4, 4, 64), dtype=torch.bfloat16, device="cuda"
        )
    ...
    def test_store_cache_4d_v_head_dim_differs(self):
        ...
        self._check_parity(
            ...
            head_num=2, head_dim=128, v_head_dim=64,
            N=16,
        )
...
class TestStoreCache4DAssertions(unittest.TestCase):
    ...
    def test_rejects_non_contiguous_view_trailing_dim(self):
        ...
        ).permute(0, 1, 3, 2)  # now shape (4, 4, 64, 4); strides broken
        v_view = torch.zeros(
            (4, 4, 4, 64), dtype=torch.bfloat16, device="cuda"
        )
