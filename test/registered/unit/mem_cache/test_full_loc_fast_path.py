...
class TestUnifiedSWARouting(unittest.TestCase):
    ...
    `loc` — the same id space post-F2; SWA layers write the swa-physical
    ...
    def test_full_layer_falls_back_to_generic_loc(self):
        """Bug regression (eval_455): fa3 x unified-SWA crashed at gpt-oss
        ...
        full_loc. Post-F2 the generic `loc` IS the full-side kernel-facing rail
...
class TestHybridLinearMLARouting(unittest.TestCase):
    ...
    def test_set_mla_kv_buffer_door_never_translates(self):
        """F2 physical-loc contract: the write door forwards `loc` UNTOUCHED.
        ...
        pool.set_mla_kv_buffer(
            layer, loc, torch.zeros(3, 1, 6), torch.zeros(3, 1, 2)
        )
    ...
    def test_get_mla_kv_buffer_door_never_translates(self):
        """F2 physical-loc contract, read side: `loc` is a read-index tensor
