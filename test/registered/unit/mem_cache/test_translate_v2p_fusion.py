...
class TestTranslateV2PFusion(unittest.TestCase):
        ...
        for page_size in (1, 4, 256):
            for multiplier in (1, 3):
                with self.subTest(page_size=page_size, multiplier=multiplier):
                    ...
                    virt = torch.tensor(
                        [0, 1, page_size, page_size + 1, 5 * page_size + (page_size - 1),
                         2 * page_size, 7 * page_size + 1, 31 * page_size],
                        dtype=torch.int64,
                        device=_DEV,
                    )
                    got = translate_v2p(
    ...
    def test_strided_input_is_handled_not_misread(self):
        ...
        self.assertTrue(torch.equal(got, want), f"got {got.tolist()} want {want.tolist()}")
