...
class TestPoolLevelSwaTranslate(unittest.TestCase):
    ...
    against a real swa-side allocator (the same stand-in recipe as the clamp
    pins in test_translate_fusion.py)."""

    def _pool_and_ids(self):
        from types import SimpleNamespace

        from test_translate_fusion import _swa_side_allocator

        swa_alloc, v = _swa_side_allocator(page_size=2)
        return SimpleNamespace(_swa_allocator=swa_alloc), v
    ...
    def test_out_int64_written_in_place_without_narrowing_round_trip(self):
        ...
        assignment)."""
        pool, v = self._pool_and_ids()
        want = UnifiedSWAKVPool.translate_loc_from_full_to_swa(pool, v).to(torch.int64)
        buf = v.clone()  # int64, aliasing-style in-place like the refill
        ret = UnifiedSWAKVPool.translate_loc_from_full_to_swa(pool, buf, out=buf)
        self.assertIs(ret, buf)
