"""
End-to-end accuracy tests for the unified memory pool on a hybrid-SWA MoE model.

Launches gpt-oss-20b with ``--enable-unified-memory`` across the dense-view
backend matrix, checking GSM8K accuracy on each. gpt-oss is a uniform-row
hybrid-SWA model, so its MHA + SWA sub-pools are DENSE per-layer views and the
fa3 cell reads them through the choke point's canonical page tables; the
resolved-default cell pins the no-pin path (the pattern that caught both
#32972 review defects — a pinned backend hides default-resolution breakage by
construction). flashinfer is absent on purpose: gpt-oss uses attention sinks,
which flashinfer does not support.
